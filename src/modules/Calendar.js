...
const currentMonthLenghtCalc = () => {
  ...
  let currentYearTake = parseFloat(
    document.getElementById("selected_year").innerHTML
  );
...
const addSubmittedStyle = el => {
  const doneDaysArray = mainArray.filter(el =>
    el.a.includes(selectedMonth + selectedYear)
  );
  const slicedDoneDaysArray = doneDaysArray.map(el => {
    return el.a.slice(7, 9);
  });
...
const calcEmptyDays = () => {
  let monthStartDay = new Date(
    selectedYear + "-" + makeTwoDigitMonthNumber() + "-01"
  ).getDay();
  if (monthStartDay === 0) {
    monthStartDay = 7;
  }
...
export const loadMonthHtml = () => {
  ...
  const addHtmlToDaysArray = daysInMonthArray.map(el => {
    return `<div style="${addPastDaysStyle(el)} ${addSubmittedStyle(
      el
    )}" day-name='dayname${el}${selectedMonth}${selectedYear}' id='calendar${el}${selectedMonth}${selectedYear}' class='day_in_month_item'>${el}</div>`;
  });
  ...
  const fullCalendarHtml = `
    ...
    `;
  document
    .getElementById("weekdaysid")
    .insertAdjacentHTML("afterend", fullCalendarHtml);
};
