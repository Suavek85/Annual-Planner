...
const view = {
  ...
  emptyRegisterForm: function() {
    ...
    [nameInputReg, passInputReg, emailInputReg].forEach = el =>
      clearItemValue(el);
  },
  ...
  displayWrongCredentials: function() {
    document
      ...
      .insertAdjacentHTML(
        "afterend",
        '<p id="signin-note">Wrong credentials</p>'
      );
  },
  ...
  displayLoading: function() {
    document
      ...
      .insertAdjacentHTML(
        "afterend",
        '<p style="color: orange" id="signin-note">Loading...</p>'
      );
  },
  ...
  displayCreatingProfile: function() {
    document
      ...
      .insertAdjacentHTML(
        "afterend",
        '<p style="color: blue" id="register-note">Creating profile...</p>'
      );
  },
  ...
  displayNoBlankFields: function() {
    document
      ...
      .insertAdjacentHTML(
        "afterend",
        '<p id="register-note">Field cannot be left blank</p>'
      );
  },
  ...
  displayUserExists: function() {
    document
      ...
      .insertAdjacentHTML(
        "afterend",
        '<p id="register-note">User already exists</p>'
      );
  },
  ...
  displayEmailNotValid: function() {
    document
      ...
      .insertAdjacentHTML(
        "afterend",
        '<p id="register-note">Invalid email format</p>'
      );
  },
  ...
  calcDateforDay: function(evTarId) {
    ...
    const currentDate = [
      evTarId.slice(8, 10),
      c,
      evTarId.slice(10, 11).toUpperCase(),
      evTarId.slice(11, 13),
      c,
      evTarId.slice(13)
    ].join("");
  ...
  removeGreenCircle: function(closeBtnPreviousIds) {
    document.getElementById(
      `${closeBtnPreviousIds[closeBtnPreviousIds.length - 1]}`
