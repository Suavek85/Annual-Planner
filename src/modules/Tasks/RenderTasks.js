...
export const displayEachTaskDay = i => {
  const mappedtasksArrayDay = mainArray[i].z.map((el, ind) => {
    return `
    <div style="display: flex; flex-direction: row; align-items: center; justify-content: space-between;background-color:${isIndexEven(
      ind
    )}">

    <div style="display: flex; flex-direction: row; align-items: center;" >

    <div style="display: inline-block; width: 45px; height: 45px; background-color: ${typeDayBackground(
      el
    ...
    <img src="images/completed.png" alt="logo done" class="icons_done" margin-left="12px" height="30px" width="30px" style="${doneTaskStyleIcon(
      el
    )}">
  ...
  document
    .getElementById("task_list_output")
    .insertAdjacentHTML("afterbegin", joinmappedtasksArrayDay);
};
...
export const displaySelectedTaskDay = (i, currentHtml) => {
  ...
  document
    .getElementById("task_list_output")
    .insertAdjacentHTML("afterbegin", joinmappedtasksArrayDay3);
};

export const displayEachTaskForm = () => {
  const mappedtasksArray = tasksArray.map(el => {
    return `<input type="checkbox" id="checkbox_todo" value="${
      el.g
    }" name="todoscb"><li style=${doneTaskStyleCross(el)} >${el.text} (${
      el.type
    })</li><img src="images/completed.png" alt="logo done" class="icons_done" height="16px" width="16px" style=${doneTaskStyleIcon(
      el
    )}><br>`;
  });
...
const typeDayArray = [
  ...
  {
    type: "Celebration",
    icon: "images/celebrate-white.png",
    bgColor: "#BC70A4"
  },
  { type: "Learning", icon: "images/learn-white.png", bgColor: "#663399" },
  {
    type: "Appointment",
    icon: "images/appointment-white.png",
    bgColor: "#A9754F"
  },
