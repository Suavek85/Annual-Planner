...
export const locationWeather = () => {
  ...
  function success(position) {
    ...
    fetch(url)
      ...
      .then(res => {
        if (mainWeatherCity) {
          mainWeatherText.innerHTML = `${Math.round(
            res.list[0].main.temp_max
          )}° in `;
          mainWeatherCity.innerHTML = `${res.city.name}`;
          mainWeatherIcon.src = `https://openweathermap.org/img/w/${
            res.list[0].weather[0].icon
          }.png`;
        }

        const mainWeatherResponsive = `<div class='more-weather-wrapper'><img  class='more-weather-icons' src='https://openweathermap.org/img/w/${
          res.list[0].weather[0].icon
        }.png'><p class='collapible-weather-text'>${Math.round(
          res.list[0].main.temp_max
        )}° in ${res.city.name} today
        </p> </div>`;
        ...
        const extraWeatherHtml = extraWeatherForArray.map(el => {
          return `<div class='more-weather-wrapper'><img id='wea_${
            el.a
          }_icon' class='more-weather-icons' src='https://openweathermap.org/img/w/${
            res.list[el.b].weather[0].icon
          }.png'><p id='wea_${
            el.a
          }' class='collapible-weather-text'>${Math.round(
            res.list[el.b].main.temp
          )}° ${expandTimes(res.list[el.b].dt)}
      </p> </div>`;
        });

        const extraWeatherHtmltoString = extraWeatherHtml.join("");
        const extraWeatherHtmltoStringResponsive =
          mainWeatherResponsive + extraWeatherHtml.join("");

        const colW = document.getElementById("collapsible_weather");

        if (colW) {
          document
            .getElementById("collapsible_weather")
            .insertAdjacentHTML("afterbegin", extraWeatherHtmltoString);
        }

        document
          .getElementById("collapsible_weather_responsive")
          .insertAdjacentHTML("afterbegin", extraWeatherHtmltoStringResponsive);
      });
  }
  ...
  function error() {
    ...
    fetch(url2)
      ...
      .then(res => {
        ...
        const extraWeatherHtml = extraWeatherForArray.map(el => {
          return `<div class=' margin: auto;'><img id='wea_${
            el.a
          }_icon' class='more-weather-icons' src='https://openweathermap.org/img/w/${
            res.list[el.b].weather[0].icon
          }.png'><p id='wea_${
