export const updateNextHolidays = () => {
  ...
  fetch(urlHols)
    ...
    .then(res => {
      ...
      const nextHolidaysResp = document.getElementById(
        "expand-holidays-responsive"
      );
      const nowHolText = `It's ${res.response.holidays[closestHolIndex].name}`;
      const closestHolText = `${daysLeft} days left till ${
        res.response.holidays[closestHolIndex].name
      }`;

      if (daysLeft === 0) {
        nextHolidays.insertAdjacentHTML(
          "afterbegin",
          nowHolText + collapsibleHols
        );
      } else {
        nextHolidays.insertAdjacentHTML(
          "afterbegin",
          closestHolText + collapsibleHols
        );
      ...
      const addExtraHolsText = el => {
        el.innerHTML = `${res.response.holidays[moreholsArray[x]].name} in ${
          moreholsDaysLeft[x]
        } days`;
        x++;
      };
      ...
      if (moreholsArray.length < 2) {
        ...
        switch (moreholsArray.length) {
          case 2:
            ...
            break;
          case 3:
            [secHol, thirdHol].forEach(el => {
              addExtraHolsText(el);
            });
            break;
          default:
            [secHol, thirdHol, fourthHol].forEach(el => {
              addExtraHolsText(el);
            });
            break;
        }
      ...
      if (moreholsArray.length < 2) {
        ...
        switch (moreholsArray.length) {
          case 2:
            ...
            break;
          case 3:
            firstHolResp.innerHTML = `${daysLeft} days left till ${
              res.response.holidays[closestHolIndex].name
            }`;
            [secHolResp, thirdHolResp].forEach(el => {
              addExtraHolsText(el);
            });
            break;
          default:
            firstHolResp.innerHTML = closestHolText;
            [secHolResp, thirdHolResp, fourthHolResp].forEach(el => {
              addExtraHolsText(el);
            });
            break;
        }
